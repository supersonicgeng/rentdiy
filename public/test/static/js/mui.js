			...
			if(hasPulldown || hasPullup) {
				...
				if(container) {
					...
					if($container.length === 1) {
						if($.os.plus && $.os.android && !window.h5pullDown) { //android 5+
							$.plusReady(function() {
...
(function($, window, undefined) {
	...
	var ajaxComplete = function(status, xhr, settings) {
		settings.complete.call(settings.context, xhr, status, settings);
	};
 ...
 */
(function($, document) {
	if(!($.os.plus && $.os.android) || window.h5pullDown) { //仅在5+android支持多webview的使用
		return;
	}
	$.plusReady(function() {
