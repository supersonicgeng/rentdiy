...
class ReportController extends Controller
{
    ...
    public function userDetail(Request $request)
    {
        ...
        $res = DB::table('user')->where($where)->paginate(20);
        foreach ($res as $k => $v){
            if($request->user_role == 1){
                ...
                $res[$k]->expense_cost_without_gts = ceil($res[$k]->landlord_expense_cost/1.15);
                $res[$k]->expense_cost_gts = $res[$k]->landlord_expense_cost-$res[$k]->landlord_expense_cost_without_gts;
                $res[$k]->discount= DB::table('expense')->where('user_id',$v->id)->where('user_cost_role',1)->sum('discount');
                $res[$k]->arrears = DB::table('user')->where('user_id',$v->id)->where('balance','<',0)->sum('balance')?DB::table('user')->where('user_id',$v->id)->where('balance','<',0)->sum('balance'):0;
                $res[$k]->charge = DB::table('charge_list')->where('user_id',$v->id)->where('charge_type',1)->where('charge_role',1)->sum('charge_fee');
                $res[$k]->balance = DB::table('user')->where('user_id',$v->id)->where('balance','>',0)->sum('balance')?DB::table('user')->where('user_id',$v->id)->where('balance','>',0)->sum('balance'):0;
            }else{
