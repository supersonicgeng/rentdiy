...
class SeedService extends CommonService
{
    public $arrXing, $numbXing;
    public $arrMing, $numbMing;
...
// 获取名字
    ...
    public function createFirstPassport(){
        Passport::create([
            'passport_id'=>23850,
            'openid'=>'oYuMH0fnWcd11tDd-ttHmNGwFXZU',
            'identity'=>1,
            'unionid'=>null,
            'groupid'=>0,
            'nickname'=>'rich蔡',
            'headimgurl'=>'https://wx.qlogo.cn/mmopen/vi_32/Q0j4TwGTfTLdSwJgCfKYnevIaYlm32FEwSS9uaowib5fuZ0JKtZQkeZW4JYQQvO0dmh5y6aofibMpbVGV6ibdO2sg/0',
            'sex'=>1,
            'subscribe'=>0,
            'country'=>'China',
            'province'=>null,
        ...
        /*Passport::create([
            'passport_id'=>23851,
            'openid'=>'oYuMH0TLex1upp9EXCR2tzG7pNMI',
            'identity'=>1,
            'unionid'=>null,
            'groupid'=>0,
            'nickname'=>'顺其自然【乐乡齐城】',
            'headimgurl'=>'https://wx.qlogo.cn/mmopen/vi_32/Q0j4TwGTfTIRgxHpXzJarZcF9gSZCRCIzyiav8bUK1RSphQPkHm3TibredWPH2hn896CloZLnGx46NWZgibw9DsTQ/0',
            'sex'=>1,
            'subscribe'=>0,
            'country'=>'China',
            'province'=>'Jiangsu',
     ...
     * 给普通管理员默认权限
     */
    public function addPermissionToManager()
    {
        $data = [];
        $pid = Permission::where('name','=','wishes')->value('id');
        service('Seed')->getChild($pid,Permission::get(),$data);
        $data[] = $pid;
        $manager = Role::where('name','=','manager')->first();
        $manager->perms()->sync($data);
    }

    //通过父id获取所有子id
    public function getChild($pid, $list, &$data)
    {
        foreach ($list as $k => $v) {
            if($v->pid == $pid){
                $data[] = $v->id;
                $this->getChild($v->id,$list,$data);
            }
        }
        return $data;
    }
