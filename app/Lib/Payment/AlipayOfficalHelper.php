...
use App\Lib\Payment\AlipayEncrypt;
use Exception;

class AlipayOfficalHelper
{
    //应用ID
    public $appId;
    //私钥文件路径
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt)
    {
        ...
        \Storage::disk('local')->put('test_alipay.json', json_encode([
               date("Y-m-d H:i:s"),
               $apiName,
               $this->appId,
               $localIp,
	...
	*/
	public function pageExecute($request,$httpmethod = "POST")
    {
		...
		if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
			...
			// 执行加密
			$enCryptContent = AlipayEncrypt::encrypt($apiParams['biz_content'], $this->encryptKey);
	...
	private function encryptJSONSignSource($request, $responseContent)
    {
		...
		$bizContent = AlipayEncrypt::decrypt($parsetItem->encryptContent, $this->encryptKey);
