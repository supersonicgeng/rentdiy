...
<div class="lockscreen-wrapper">
    ...
    <!-- User name -->
    <div class="lockscreen-name">{{auth()->user()->real_name}}</div>

    <!-- START LOCK SCREEN ITEM -->
    <div class="lockscreen-item">
        <!-- lockscreen image -->
        <div class="lockscreen-image">
            <img src="{{auth()->user()->avatar ?? '/avatar.png'}}" alt="User Image">
        </div>
        <!-- /.lockscreen-image -->

        <!-- lockscreen credentials (contains the form) -->
        <form class="lockscreen-credentials" method="post" action="{{route('admin.lock.login')}}">
            @csrf
